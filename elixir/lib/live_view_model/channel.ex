defmodule LiveViewModel.Channel do
  @moduledoc """
  To build a LiveViewModel application, you'll first want to add a channel that `use`s this module.

  ```
    use LiveViewModel.Channel, web_module: MyAppWeb, max_version: 100
  ```
  - `message_builder` optional, defaults to `{LiveViewModel.MessageBuilder, ignore_keys: [:__meta__]}`. If
  ...
  See `LiveViewModel.MessageBuilder` for details
  ...
  can contain a single `LiveViewModel.Event` or a list of events as well as the new state.
  """
  @callback handle_event(event_name :: binary(), payload :: term(), state :: term()) ::
              {:reply, reply :: %LiveViewModel.Event{} | list(%LiveViewModel.Event{}),
               new_state :: any()}
  ...
  you need to send events to the client, otherwise return `:noreply`.  `:reply` tuples
  can contain a single `LiveViewModel.Event` or a list of events, as well as the new state and
    ...
    quote do
      ...
      @message_builder unquote(
                         Keyword.get(
                           opts,
                           :message_builder,
                           {LiveViewModel.MessageBuilder, ignore_keys: [:__meta__]}
                         )
                       )
      ...
      def handle_in("lvm_evt:" <> event_name, payload, %{assigns: assigns} = socket) do
